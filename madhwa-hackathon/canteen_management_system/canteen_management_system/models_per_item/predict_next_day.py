...
# helper: generate lag/rolling features for each item
...
for file in os.listdir(MODEL_DIR):
    if file.startswith("lgb_item_") and file.endswith(".pkl"):
        ...
        # take last row as reference
        last_row = item_df.iloc[-1:].copy()

        # create next day's feature row
        ...
        # compute lag and rolling using the latest actual data
        for lag in [1, 2, 3, 7, 14]:
            if len(item_df) >= lag:
        ...
        # use next_row for prediction (not last_row)
